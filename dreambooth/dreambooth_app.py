...
class SharedConfig:
    ...
    #model_name: str = "sayakpaul/civitai-light-shadow-lora"
...
def train(instance_example_urls):
    ...
    #img_path = 'C:/Users/anubh/Silly hacks 2024/frames_output'
    # modal.interact()
    # import IPython
    # IPython.embed()
...
class Model:
    ...
    def inference(self, text, config):
        print('AG: going into inference now...')
        # image = self.pipe(
        #     text,
        ...
        # Process the generated image and get the image and audio responses
        print('AG: processing audio ')
        audio = process_image_and_return_audio.remote(image1_data, image1_media_type)
        print(type(audio))
        return image, audio
...
def process_image_and_return_audio(image_data, image_type):
    response = generate.remote(image_data, image_type)
    return response
@stub.function(
    image=image,
    timeout=1800,  # 30 minutes
)
def generate(image_data, image_type):
    llm = Zephyr()
    tts = Tortoise()
    tts_enabled = True
    """print("I'm here")
    ...
    if image_data and image_type:
        # Call LLM generate method with image data and type
        print('AG: sending data to LLM')
        text_result = llm.generate.remote_gen(image_data, image_type)
        print('AG: data received from LLM')
        # Generate audio from the text using TTS
        audio_result = tts.speak(text_result)
        print('AG: audio generated')
        # Save audio to file
        filename = "generated_audio.wav"  # You can use a unique filename here
        with open(filename, "wb") as audio_file:
            audio_file.write(audio_result.getvalue())
        print('AG: audio saved')
        return filename
...
def fastapi_app():
    ...
    def go(text=""):
        ...
        return Model().inference.remote("Make a hyper realistic anime style image of Anubhav the Person with bright colours. Give Anubhav a big smile. You have to make sure that the image invokes positive emotions in the viewers. "+text, config)
...
#        f"a painting of {instance_phrase.title()} With A Pearl Earring, by Vermeer",
    ...
    with gr.Blocks(
        theme=theme, css=css, title="Talking Toons"
    ) as interface:
        gr.Markdown(
            f"# Silly hacks 2024. Dream up images of {instance_phrase}.\n\n{description}",
        )
...
def run():
    ...
    print('AG:::going into training now...')
